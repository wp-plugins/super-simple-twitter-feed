...
/*
  ...
  /*
	...
	if ((!($sstf_transient) || (trim(esc_attr(get_option('sstf_consumer_twitter_cache_time')))=='0'))){
		...
		$sstfNameLink = trim(esc_attr(get_option('sstf_consumer_twitter_name_link')));
		
		foreach($twitter_data as &$tData){
			$date = date('M j',strtotime($twitter_data[$t]['created_at']));
			//echo $twitter_data[$t]['created_at'];

			if($sstfNameLink == "yes"){
				$sstfTweet = '<a href="https://twitter.com/'.$consumer.'">@'. $consumer . '</a> - '.$date.'<br />';
			}
			else {
				$sstfTweet = '';
			}	
...
function sstf_init(){
	...
	register_setting('sstf_options','sstf_consumer_twitter_name_link');
} 
	...
	<div class="wrap">
		...
		<form action="options.php" method="post" id="sstf-options-form">
			...
			<table class="form-table">
				 ...
				 */
				 $currentNameLink = trim(esc_attr(get_option('sstf_consumer_twitter_name_link')));
				 $nameLinkArray   = array(
				 	"yes","no"
				 );
				?>
				<tr class="even" valign="top">
					<th scope="row"><label for="sstf_consumer_twitter_name_link">Link Back to Twitter: </label></th>
					<td><select id="sstf_consumer_twitter_name_link" name="sstf_consumer_twitter_name_link">
							<?php 
								foreach($nameLinkArray as &$link){
									if($currentNameLink==$link){
						...
						<p class="description">Provides your twitter username as a link back to your feed above each post</p>
					</td>
				</tr>
				<?php
				/*
